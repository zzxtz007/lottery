...
let TOTAL_CARDS,
    btns = {
        animate: document.querySelector("#container"),
...
function initAll() {
    window.AJAX({
        ...
        success(data) {
            ...
            let lastLucky = basicData.luckyUsers[1]
            let needComplete = basicData.prizes[0].count === 999
            let luckyCount = (lastLucky ? lastLucky.length : 0);
            if (luckyCount === basicData.prizes[1].count && needComplete) {
                btns.lottery.innerHTML = "抽奖完成";
                lottingCompele = true
            }
        }
    });
    ...
    document.querySelector("#menu").addEventListener("click", function (e) {
        ...
        if (isLotting) {
            if (e.target.id === "lottery") {
                rotateObj.stop();
                //todo 如果后续没用无限抽奖环节 应该改为 抽奖完成
                console.log(currentPrize, basicData)
                if (currentPrizeIndex === 1) {
                    let needComplete = basicData.prizes[0].count === 999
                    let luckys = basicData.luckyUsers[currentPrize.type];
                    let luckyCount = (luckys ? luckys.length : 0) + EACH_COUNT[currentPrizeIndex];
                    if (luckyCount === basicData.prizes[currentPrizeIndex].count && needComplete) {
                        console.log(`当前奖品 抽奖结束`)
                        btns.lottery.innerHTML = "抽奖完成";
                        lottingCompele = true
                        return false
                    }
...
function switchScreen(type) {
    switch (type) {
        case "enter":
            btns.baseBar.classList.remove("none");
            btns.animate.classList.remove("none")
            btns.text.classList.add("none")
            btns.enter.classList.remove("none");
            btns.lotteryBar.classList.add("none");
            transform(targets.table, 2000);
            break;
        case "end":
            btns.animate.classList.add("none")
            btns.text.classList.remove("none")
            btns.baseBar.classList.add("none");
            break;
        default:
            btns.baseBar.classList.remove("none");
            btns.animate.classList.remove("none")
